...
#    if max(inputFile[i-1].dropna().tolist()) > 60:
#        text(i,55,'%d' % max(inputFile[i-1].dropna().tolist()),horizontalalignment='center',color='#0266c8',size=12)
...
for i in range(1,len(medians) + 1):
    text(i,medians[i-1]+50,'%.2f' % medians[i-1],horizontalalignment='right',color='red',size=12)
...
for i in range(1,len(mean) + 1):
    text(i,mean[i-1]-50,'%.2f' % mean[i-1],horizontalalignment='right',color='purple',size=12)
...
plt.axvline(x=1.5,color='k', linestyle='-', lw=2)
plt.axvline(x=3.5,color='k', linestyle='-', lw=2)
plt.axvline(x=6.5,color='k', linestyle='-', lw=2)
plt.axvline(x=10.5,color='k', linestyle='-', lw=2)
