...
for j in range(0,6):
	...
	if j == 0:
		...
		for i in [1,250,500,1000,2000]:
			if i==1:
				f = open('data/distributed/{}/masterClient.log'.format(worker),'r')
			else:
				f = open('data/distributed/{}/{}/masterClient.log'.format(worker,i),'r')
			data = f.readlines()
			f.close()
...
ax1.set_yticklabels(np.arange(0,180001,30000),size=font_size-5)
...
ax2.set_yticklabels(np.arange(0,11,2),size=font_size-5)
